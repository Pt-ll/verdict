import type { Contest, Standings, Submission, Verdict } from '../model';
...
function renderDetails(
  ...
  return [
    ...
    '<h2>提交详情</h2>',
    '<p class="hint" id="verdict-hint">点上面的分数单元格查看逐测试点结果。</p>',
...
(function () {
  ...
  JSON.parse(source.textContent || '[]').forEach(function (item) {
    byCell[item.contestant + '\\u0000' + item.problem] = item;
  });

  function text(value) { return String(value == null ? '' : value); }

  function render(detail) {
    var rows = detail.cases.map(function (item) {
      var memory = item.memoryKb > 0 ? (item.memoryKb / 1024).toFixed(1) + 'MB' : 'n/a';
      return '<tr><td>' + text(item.test) + '</td><td>' + text(item.verdict) + '</td>' +
        '<td>' + text(item.timeMs) + 'ms</td><td>' + memory + '</td>' +
        '<td class="name">' + text(item.message) + '</td></tr>';
    }).join('');
    panel.innerHTML = '<p><strong>' + text(detail.contestant) + ' · ' + text(detail.problem) +
      '</strong>：' + text(detail.score) + '/' + text(detail.maxScore) + ' 分 ' +
      text(detail.verdict) + (detail.message ? '（' + text(detail.message) + '）' : '') + '</p>' +
      (rows ? '<table class="detail"><thead><tr><th>测试点</th><th>判定</th><th>用时</th>' +
        '<th>内存</th><th class="name">说明</th></tr></thead><tbody>' + rows + '</tbody></table>' : '');
  }
