...
1. 命令面板执行 `Verdict: 评测全部` — 输出通道给出 `alice 300 / bob 230`，状态栏跟着走进度
2. `Verdict: 显示榜单` — 分数矩阵（点任意单元格展开逐测试点结果）、各题情况、分数分布
3. `Verdict: 导出 HTML 成绩` — 得到一个可以直接发给别人的成绩单，不依赖网络
4. 打开 `players/bob/A.cpp`（故意在大数据上写错的那份），Testing 面板里点调试按钮，看它算错在哪

### 做自己的题

1. `Verdict: 新建比赛` → 填比赛 id、标题、重测上限、选手
2. `Verdict: 新建题目` → 填题目 id、时限、内存
3. 把 `1.in` / `1.out`（可以有很多组）放进题目的 `data/` 目录
4. `Verdict: 导入测试点` → `Verdict: 配置子任务`（均分或手改 `problem.json`）
...
还没选许可证。要发布到插件市场或让别人使用，需要你决定一个（MIT 与 Apache-2.0 是这类工具的常见选择），
放到仓库根的 `LICENSE` 文件里，并在 `package.json` 里加上 `"license"` 字段。
